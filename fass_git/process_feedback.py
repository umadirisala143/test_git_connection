...
def process_feedback(config,**doc_object):
    ...
    for entity in entities:
        ...
        for attr in attributes:
            if attr['name'] == 'recommendation':
                if 'Patient will not' in attr['value'].lower():
                    TARGET = 0
                else:
                    TARGET = 1
    ...
    input_dict["TARGET"] = TARGET if TARGET else 1
    overfit_count = 20
    final_data = []
    for i in range(overfit_count):
        final_data.append(input_dict)
